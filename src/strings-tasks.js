...
function removeFirstOccurrences(str, value) {
  ...
  const ind = lowerStr.indexOf(lowerValue);
...
function removeLastOccurrences(str, value) {
  const lowerStr = str.toLowerCase();
  const lowerValue = value.toLowerCase();
  const lastIndex = lowerStr.lastIndexOf(lowerValue);
...
function isPalindrome(str) {
  return (
    str
      ...
      .split('')
      .reverse()
      .join('') === str.toLowerCase().replace(/[^a-z]/g, '')
  );
}
...
function encodeToRot13(str) {
  const encryptedAlph = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const alph = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  let str1 = '';
  for (let i = 0; i < str.length; i += 1) {
    if (alph.indexOf(str[i]) !== -1) {
      str1 += encryptedAlph[alph.indexOf(str[i])];
    } else {
      str1 += str[i];
    }
  }
...
function getCardId(value) {
  ...
  if (value[value.length - 1] === '♣') {
    result = str1.indexOf(value);
  }
  if (value[value.length - 1] === '♦') {
    result = str2.indexOf(value) + 13;
  }
  if (value[value.length - 1] === '♥') {
    result = str3.indexOf(value) + 26;
  }
  if (value[value.length - 1] === '♠') {
    result = str4.indexOf(value) + 39;
  }
  return result;
}
